...
public class TelaPrincipal extends JFrame {
    ...
    private JPanel criarTitulo() {
        ...
        btnLogout.setBorder(new EmptyBorder(8, 18, 8, 18));

        btnLogout.addActionListener(e -> {
            int confirm = JOptionPane.showConfirmDialog(
                    this, "Deseja sair da conta?", "Logout",
    ...
    private JPanel criarFormulario() {
        ...
        // Campos dentro de um scroll
        JPanel campos = new JPanel();
        campos.setLayout(new BoxLayout(campos, BoxLayout.Y_AXIS));
        campos.setBackground(COR_PAINEL);
        campos.setBorder(new EmptyBorder(16, 16, 16, 16));

        txtNome = adicionarCampo(campos, "Nome completo");
        txtEmail = adicionarCampo(campos, "Email");
        txtTelefone = adicionarCampo(campos, "Telefone");
        txtCpf = adicionarCampo(campos, "CPF");
        txtRua = adicionarCampo(campos, "Rua");
        txtNumero = adicionarCampo(campos, "Número");
        txtBairro = adicionarCampo(campos, "Bairro");
        txtCidade = adicionarCampo(campos, "Cidade");
        txtDataNasc = adicionarCampo(campos, "Data Nasc. (dd/mm/aaaa)");
        ...
        scrollCampos.setBorder(BorderFactory.createLineBorder(new Color(220, 220, 235), 1));
        scrollCampos.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
        scrollCampos.getVerticalScrollBar().setUnitIncrement(12);

        // Botões
        JPanel botoes = new JPanel(new GridLayout(2, 2, 8, 8));
        botoes.setOpaque(false);
        botoes.setBorder(new EmptyBorder(8, 0, 0, 0));
    ...
    private JTextField adicionarCampo(JPanel pai, String labelTxt) {
        ...
        pai.add(label);
        pai.add(Box.createVerticalStrut(2));
        pai.add(field);
        pai.add(Box.createVerticalStrut(10));
        return field;
    }
    ...
    private void buscar() {
        try {
            popularTabela(dao.buscarPorNome(txtBusca.getText()));
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Erro: " + ex.getMessage());
        }
    }
